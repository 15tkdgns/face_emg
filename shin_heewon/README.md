...
cp configs/aihub_A_densenet121_ce.yaml shin_heewon/config.yaml

# 2. Edit shin_heewon/config.yaml:
#    output_dir: shin_heewon/results

# 3. Train
python experiment.py --config shin_heewon/config.yaml
```

### Option B — write your own training script
```bash
python shin_heewon/train_custom.py   # if you write one
```

---

## Rules
- Only edit files inside your own folder + `shin_heewon/model.py`
- Do NOT modify shared root files (dataset.py, train.py, etc.)
- Save your best checkpoint to `shin_heewon/results/best_model.pth`
- Record final test accuracy and F1 in `shin_heewon/results/result.json`
